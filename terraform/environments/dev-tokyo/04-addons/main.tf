...
data "terraform_remote_state" "eks" {
  ...
  config = {
    bucket = "utterai-dev-terraform-state"
    key    = "dev-tokyo/platform/terraform.tfstate"
    region = "ap-northeast-2"
  }
}
...
data "terraform_remote_state" "services" {
  ...
  config = {
    bucket = "utterai-dev-terraform-state"
    key    = "dev-tokyo/services/terraform.tfstate"
    region = "ap-northeast-2"
  }
}
...
# pod_subnet_az_map = { "ap-northeast-1a" = subnet-xxx, "ap-northeast-1c" = subnet-yyy }
...
data "aws_lb" "api" {
  ...
  tags = {
    ...
    "ingress.k8s.aws/stack" = "utterai-dev-tokyo"
  }
}
...
module "cloudfront" {
  ...
  alb_dns_name        = data.aws_lb.api[0].dns_name
}
